...
def read_grid(file_path):
    ...
    for line_of_text in myfile:
        pixel_grid.append(csv_line_to_list(line_of_text))
    return pixel_grid
    myfile.close()    
...
def get_pixel_at(pixel_grid, i, j):
    ...
    '''
    len_row = len(pixel_grid)-1
    len_column = len(pixel_grid[0])-1
    if i > len_row or j > len_column:
        return 0
    elif i < 0 or j < 0:
        return 0
    else:
    ...
    for column in range(-1,2,1):
        for row in range (-1,2,1):
            if i+row < 0 or j+column < 0:
                0
            elif i+row > len_row or j+column > len_column:
                0
            else:
                pixel_sum += pixel_grid[i+row][j+column]
...
def blur(pixel_grid):
    ...
    for i in range(r):       
        new_pixel_grid.append([])
        for j in range(c):   
            a = average_of_surrounding(pixel_grid, i, j)
            # new_pixel_grid[i][j] = a
            new_pixel_grid[i].append(a)
...
if len(sys.argv) != 2:
    ...
    print "   <input_file>_blur.png (an image) and"
    print "   <input_file>_blur_grid.txt (a CSV-formatted text file)."
    sys.exit()
...
# the input file.
out_image_filename = input_filename + '_blur.png'
out_grid_filename = input_filename + '_blur_grid.txt'
...
write_image(out_image_filename, blurred_image)
write_grid(out_grid_filename, blurred_image)
